...
(() => {
  const STYLE = `
    .student-name-cell{min-width:150px!important;width:150px!important;max-width:150px!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;}
    .student-name-cell *{max-width:100%;}
    .student-photo-thumb{width:34px;height:34px;border-radius:50%;object-fit:cover;display:inline-block;vertical-align:middle;margin-right:7px;border:2px solid #e6ebf2;background:#f4f7fb;flex:none;}
  ...
  function renderRowPhoto(id,photo){
    document.querySelectorAll('#studentsTableBody tr').forEach(row=>{
      ...
      nameCell.classList.add('student-name-cell');
      nameCell.title=nameCell.textContent.trim();
      const existing=nameCell.querySelector('.student-photo-thumb,.student-photo-placeholder');
      existing?.remove();
      if(photo){
  ...
  function scanRows(){
    document.querySelectorAll('#studentsTableBody tr').forEach(async row=>{
      ...
      nameCell.classList.add('student-name-cell');
      nameCell.title=nameCell.textContent.trim();
      if(row.dataset.photoChecked==='1')return;
      row.dataset.photoChecked='1';
      try{
        const snap=await getDoc(doc(db,'students',id));
        const data=snap.exists()?snap.data():null;
        renderRowPhoto(id,data?.photo||'');
      }catch(e){console.warn('Student photo read failed',e);}
    });
  }
  ...
  document.addEventListener('click',e=>{
    const btn=e.target.closest('.id-card-btn');
    if(btn) pendingStudentId=btn.dataset.id||'';
  },true);
    ...
    if(child && id){
      setTimeout(async()=>{
        try{
          ...
          if(target){
            target.innerHTML=`<img src="${photo}" alt="Student Photo" style="width:100%;height:100%;object-fit:cover;border-radius:50%;display:block">`;
