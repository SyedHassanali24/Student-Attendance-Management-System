...
/* SSHACMS — SAFE ATTENDANCE ENHANCER
   ...
   browser freeze that occurred after the responsive update.
*/
(() => {
  ...
  function message(text, error=false) {
    const n = $("safeScannerMessage");
    if (n) { n.textContent = text; n.style.color = error ? "#b42318" : "#667085"; }
  }

  function addStyles() {
    if ($("safeAttendanceStyles")) return;
    const s = document.createElement("style");
    s.id = "safeAttendanceStyles";
    s.textContent = `
  ...
  function addUI() {
    ...
    if (!page || $("safeAttendanceControls")) return;
    const wrap = document.createElement("div");
    wrap.id = "safeAttendanceControls";
    wrap.innerHTML = `
      <div class="sa">
        <div class="sa-head"><div><span class="sa-kicker">ATTENDANCE CONTROL CENTER</span><h3>Check-in, Check-out & Daily Exceptions</h3><p>First scan = arrival, second scan = departure. 15-minute late rule is automatic.</p></div><span class="sa-live">● Live</span></div>
        <div class="sa-grid">
        ...
        <div class="sa-summary" id="safeSummary"></div>
      </div>`;
    ...
    $("safeOffDate").value = today();
    $("safeStart").onclick = start;
    $("safeStop").onclick = stop;
    $("safeSwitch").onclick = switchCamera;
    $("safeLeaveBtn").onclick = markLeave;
    $("safeOffBtn").onclick = markOff;
    $("startScannerBtn")?.style.setProperty("display","none");
    $("stopScannerBtn")?.style.setProperty("display","none");
  }

  async function start() {
    if (running) return;
    if (typeof Html5Qrcode === "undefined") return message("QR scanner library is not ready. Refresh once and try again.", true);
    try {
      await stop();
      ...
      running = true;
      updateCameraLabel();
      message(`Scanner active — ${camera === "environment" ? "rear" : "front"} camera.`);
    } catch (e) {
      console.error("Safe attendance camera error:", e);
      running = false; scanner = null;
      message("Unable to start camera. Allow camera permission and use HTTPS.", true);
    }
  }
  ...
  function updateCameraLabel() {
    const n = $("safeCameraLabel");
    if (n) n.textContent = camera === "environment" ? "Rear camera selected" : "Front camera selected";
  }
  ...
  async function onScan(decodedText) {
    ...
    let parsed;
    try { parsed = JSON.parse(String(decodedText).trim()); }
    catch (_) { parsed = { studentId:String(decodedText).trim() }; }
    ...
    const existing = records.find(r => r.date === date && (r.studentDocId === student.id || r.studentId === student.studentId));
    const d = new Date();

    try {
      if (!existing) {
        const st = status(student, d);
        await addDoc(collection(db,"attendance"), {
          studentDocId:student.id, studentId:student.studentId || "", name:student.name || "", fatherName:student.fatherName || "", phone:student.phone || "", course:student.course || "", batch:student.batch || "",
          date, time:clock(d), checkInTime:clock(d), checkOutTime:"", status:st, attendanceMode:"check-in-out", scheduleStart:schedule(student).label, timestamp:serverTimestamp()
        });
        message(`${student.name} checked in at ${clock(d)} — ${st}.`);
      } else if (!existing.checkOutTime) {
        await updateDoc(doc(db,"attendance",existing.id), { checkOutTime:clock(d), exitStatus:"Checked Out", lastUpdatedAt:serverTimestamp() });
        message(`${student.name} checked out at ${clock(d)}.`);
      } else {
        message(`${student.name} already has check-in and check-out today.`);
      }
    } catch (e) { console.error(e); message("Attendance error: " + e.message, true); }
  }

  async function isDayOff(date) {
    try { return !(await getDocs(query(collection(db,"coachingDays"),where("date","==",date)))).empty; }
    catch (_) { return false; }
  }
  async function isOnLeave(id,date) {
    try { return !(await getDocs(query(collection(db,"studentLeaves"),where("studentDocId","==",id),where("date","==",date)))).empty; }
    catch (_) { return false; }
  }

  async function markLeave() {
    const id=$("safeLeaveStudent")?.value, date=$("safeLeaveDate")?.value || today(), reason=$("safeLeaveReason")?.value.trim() || "Personal leave";
    const s=students.find(x=>x.id===id); if(!s)return alert("Please select a student.");
    try { if(await isOnLeave(s.id,date))return alert("Leave is already marked."); await addDoc(collection(db,"studentLeaves"),{studentDocId:s.id,studentId:s.studentId||"",name:s.name||"",date,reason,status:"Leave",createdAt:serverTimestamp()}); alert(`${s.name} marked on leave for ${date}.`); }
    catch(e){ alert("Unable to mark leave: "+e.message); }
  }
  ...
  async function markOff() {
    ...
    try { if(await isDayOff(date))return alert("Off day already exists."); await addDoc(collection(db,"coachingDays"),{date,type,reason,country:type === "government_holiday" ? "Pakistan" : "",createdAt:serverTimestamp()}); alert(`${reason} saved for ${date}.`); }
    catch(e){ alert("Unable to save: "+e.message); }
  }

  function fillLeaveStudents() {
    const select=$("safeLeaveStudent"); if(!select)return;
